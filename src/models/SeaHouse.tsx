...
const SeaHouse: React.FC<SeaHouseProps> = ({ isRotating, setIsRotating, setCurrentStage, ...props }) => {
    ...
    const handlePointerDown = useCallback((event: PointerEvent | TouchEvent) => {
        ...
        setIsRotating(true);
        lastX.current = 'touches' in event ? event.touches[0].clientX : event.clientX;
    }, [setIsRotating]);
    ...
    useFrame(() => {
        ...
        const normalizedRotation = ((seaHouseRef.current.rotation.y % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI);
    ...
    return (
        <group ref={seaHouseRef} {...props} dispose={null}>
            <group scale={0.1}>
                <group position={[2.045, 0.446, -0.755]} rotation={[0.018, -0.014, -0.013]} scale={17.555}>
                    {['ChimneyMaterial', 'PineAppleMaterial', 'DoorMaterial', 'DoorWheelMaterial', 'WindowMaterial', 'CrownMaterial'].map((material) => (
                        <mesh
                            key={material}
                            castShadow
                            receiveShadow
